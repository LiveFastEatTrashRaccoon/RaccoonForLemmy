...
internal class DefaultMultiCommunityRepository(private val dao: MultiCommunityDao) : MultiCommunityRepository {
    override suspend fun getAll(accountId: Long): List<MultiCommunityModel> = dao
        .getAll(accountId)
        .executeAsList()
        .map { it.toModel() }

    override suspend fun getById(id: Long): MultiCommunityModel? = dao
        .getById(id)
        .executeAsOneOrNull()
        ?.toModel()
    ...
    override suspend fun update(model: MultiCommunityModel): Unit = dao.update(
        name = model.name,
        icon = model.icon,
        communityIds = model.communityIds.joinToString(","),
        id = model.id ?: 0,
    ...
    override suspend fun delete(model: MultiCommunityModel): Unit = dao.delete(model.id ?: 0)
}
