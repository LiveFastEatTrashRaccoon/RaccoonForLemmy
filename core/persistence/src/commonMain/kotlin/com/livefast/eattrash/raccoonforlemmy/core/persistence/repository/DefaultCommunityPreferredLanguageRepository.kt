...
internal class DefaultCommunityPreferredLanguageRepository(private val keyStore: TemporaryKeyStore) :
    ...
    override suspend fun save(handle: String, value: Long?) {
        val map = deserializeMap()
        if (value != null) {
            map[handle] = value
        } else {
    ...
    private suspend fun deserializeMap(): MutableMap<String, Long> = keyStore
    ...
    private fun serializeMap(map: Map<String, Long>): List<String> = map.map { e ->
        e.key + ":" + e.value
    }
