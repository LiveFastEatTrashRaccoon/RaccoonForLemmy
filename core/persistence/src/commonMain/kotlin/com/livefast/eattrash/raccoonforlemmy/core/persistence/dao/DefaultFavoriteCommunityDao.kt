...
internal class DefaultFavoriteCommunityDao(
    private val queries: FavoritecommunitiesQueries,
) : FavoriteCommunityDao {
    override fun getAll(accountId: Long?): Query<FavoriteCommunityEntity> {
        return queries.getAll(accountId)
    }

    override fun getBy(communityId: Long, accountId: Long?): Query<FavoriteCommunityEntity> {
        return queries.getBy(communityId, accountId)
    }
