...
class DefaultDraftRepository(
    ...
    override suspend fun getAll(type: DraftType, accountId: Long): List<DraftModel> =
    ...
    override suspend fun getBy(id: Long): DraftModel? = withContext(Dispatchers.IO) {
        db.draftsQueries.getBy(id).executeAsOneOrNull()?.toModel()
    }

    override suspend fun create(model: DraftModel, accountId: Long) = withContext(Dispatchers.IO) {
    ...
    override suspend fun update(model: DraftModel) = withContext(Dispatchers.IO) {
