 ...
 *
 * @link https://issuetracker.google.com/issues/252334353
 *
 * CREDITS:
 * https://gist.githubusercontent.com/Joe3112/580d66f633c0f86c37e3f122092bf93e/raw/b81ffd283185edde0d5f842e776432f259623dcf/NoFlingSwipeToDismiss.kt
 */
...
/**
 * Create and [remember] a [NoFlingSwipeableV2State].
 *
 ...
 *
 * When a swipe is detected, the offset of the [NoFlingSwipeableV2State] will be updated with the swipe
 * delta. You should use this offset to move your content accordingly (see [Modifier.offset]).
 * When the swipe ends, the offset will be animated to one of the anchors and when that anchor is
 * reached, the value of the [NoFlingSwipeableV2State] will also be updated to the value corresponding to
 * the new anchor.
 *
 * Swiping is constrained between the minimum and maximum anchors.
 *
 * @param state The associated [NoFlingSwipeableV2State].
...
/**
 * Define anchor points for a given [NoFlingSwipeableV2State] based on this node's layout size and update
 * the state with them.
 *
 * @param state The associated [NoFlingSwipeableV2State]
 * @param possibleValues All possible values the [NoFlingSwipeableV2State] could be in.
...
/**
 * State of the [NoFlingSwipeToDismiss] composable.
 *
 * @param initialValue The initial value of the state.
 * @param confirmValueChange Optional callback invoked to confirm or veto a pending state change.
 * @param positionalThreshold The positional threshold to be used when calculating the target state
...
class NoFlingDismissState(
    ...
    /**
     * The current state value of the [NoFlingSwipeableV2State].
     */
     ...
     * change the background of the [NoFlingSwipeToDismiss] if you want different actions on each side.
     */
    ...
    suspend fun dismiss(direction: DismissDirection) {
        val targetValue =
            if (direction == DismissDirection.StartToEnd) DismissValue.DismissedToEnd else DismissValue.DismissedToStart
        swipeableState.animateTo(targetValue = targetValue)
    }

    companion object {
        /**
         * The default [Saver] implementation for [NoFlingSwipeableV2State].
         */
        fun Saver(
 ...
 * [NoFlingSwipeableV2State] use [rememberNoFlingDismissState].
 *
 * @param initialValue The initial value of the state.
 ...
 * [positionalThreshold] and [fixedPositionalThreshold] methods.
...
internal class NoFlingSwipeableV2State<T>(
    ...
    private val swipeMutex = MutatorMutex()
    ...
    /**
     * The current value of the [NoFlingSwipeableV2State].
     */
    ...
    /**
     * Swipe by the [delta], coerce it in the bounds and dispatch it to the [NoFlingSwipeableV2State].
     *
     * @return The delta the consumed by the [NoFlingSwipeableV2State]
     */
    ...
    private fun trySnapTo(targetValue: T): Boolean {
        return runBlocking {
            withTimeoutOrNull(500) {
                swipeMutex.mutate { snap(targetValue) }
                true
            } ?: false
        }
    }
 ...
 * anchor that needs to be reached for [NoFlingSwipeableV2State] to settle to the next closest anchor.
 *
 * @see [threshold] for a fractional positional threshold
 */
@ExperimentalMaterial3Api
...
/**
 * Contains useful defaults for [swipeableV2] and [NoFlingSwipeableV2State].
 */
@Stable
@ExperimentalMaterial3Api
internal object NoFlingSwipeableV2Defaults {
    /**
     * The default animation used by [NoFlingSwipeableV2State].
     */
    @ExperimentalMaterial3Api
    val AnimationSpec = SpringSpec<Float>()

    /**
     * The default velocity threshold (1.8 dp per millisecond) used by [rememberNoFlingDismissState].
     */
    @ExperimentalMaterial3Api
    val VelocityThreshold: Dp = 125.dp

    /**
     * The default positional threshold (56 dp) used by [rememberNoFlingDismissState]
     */
    @ExperimentalMaterial3Api
     ...
     * delegate to [NoFlingSwipeableV2State].
     *
     * @param state The [NoFlingSwipeableV2State] the change handler will read from
 ...
 *
 * @see NoFlingSwipeableV2Defaults.ReconcileAnimationOnAnchorChangeHandler for a default implementation
 */
@ExperimentalMaterial3Api
internal fun interface AnchorChangeHandler<T> {

    /**
     * Callback that is invoked when the anchors have changed, after the [NoFlingSwipeableV2State] has been
